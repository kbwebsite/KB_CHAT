...
async def forward_message(
    ...
    for cid in target_ids:
        if (
            not db.query(ConversationMember)
            .filter_by(conversation_id=cid, user_id=current_user.id)
            .first()
        ):
            continue
        new_msg = Message(
    ...
    for cid in created:
        ...
        if last:
            ...
            await manager.broadcast_to_conversation(
                cid,
                {"type": "message.new", "payload": payload_ws},
                member_ids=member_ids,
            )
    return success_response(
        {"forwarded_to": created}, f"Forwarded to {len(created)} conversations"
    )
...
def clear_chat(
    ...
    max_id = (
        db.query(_func.max(Message.id)).filter_by(conversation_id=conv_id).scalar() or 0
    )
    # Per-user clear: hide everything up to now FOR ME (no data deleted for
    # anyone else) and drop phantom unreads.
    membership.cleared_before_id = max_id
    if (membership.last_read_message_id or 0) < max_id:
        membership.last_read_message_id = max_id
    deleted_count = 0
    if scope == "all":
        # "all" additionally soft-deletes for everyone; owner-gated in groups.
        conv = db.query(Conversation).filter_by(id=conv_id).first()
        if conv and conv.is_group and membership and membership.role != "owner":
            raise HTTPException(
                status_code=403, detail="Only group owner can clear all messages"
            )
        deleted_count = (
            db.query(Message)
    ...
    db.commit()
    return success_response(
        {"cleared_before_id": max_id, "deleted_count": deleted_count}, "Chat cleared"
    )


def _blocked_pair(db: Session, a: int, b: int) -> bool:
    """True when either user blocked the other (1-1 chats only)."""
    return (
...
def block_contact(
    ...
    if not existing:
        ...
        db.commit()
    return success_response({"blocked": user_id}, "Contact blocked")


@router.post("/contacts/{user_id}/unblock")
def unblock_contact(
    user_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
    ...
    db.commit()
    return success_response({"unblocked": user_id}, "Contact unblocked")


@router.get("/contacts/blocked")
def blocked_list(
    db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    rows = db.query(BlockedUser).filter_by(blocker_id=current_user.id).all()
    result = []
    for r in rows:
        u = db.query(User).filter_by(id=r.blocked_id).first()
        if not u:
            continue
        result.append(
            {
...
def export_chat(
    ...
    if format == "txt":
        ...
        lines.append(
            f"Members: {', '.join([m['display_name'] for m in export_members])}"
        )
        lines.append("-" * 40)
        for em in export_msgs:
            lines.append(
                f"[{em['timestamp']}] {em['display_name']} (@{em['sender']}): {em['content']}"
            )
            if em["attachments"]:
                for a in em["attachments"]:
                    lines.append(
                        f"  [Attachment: {a['filename']} ({a['mime_type']}, {a['size']} bytes)]"
                    )
        txt_content = "\n".join(lines)
        ...
        return PlainTextResponse(
            txt_content,
            headers={
                "Content-Disposition": f"attachment; filename=kbchat_{conv_id}.txt"
            },
        )
    else:
        return success_response(
    ...
    return success_response(
        {
            ...
            "muted_until": mem.muted_until.isoformat() if mem.muted_until else None,
        },
        "Mute updated",
    )
...
def set_profile_status(
    ...
    if payload.get("expires_in"):
        ...
        if hours > 0:
            current_user.status_expires_at = datetime.now(timezone.utc) + timedelta(
                hours=hours
            )
    db.commit()
    return success_response(
        {
            ...
            "status_expires_at": current_user.status_expires_at.isoformat()
            if current_user.status_expires_at
            else None,
        },
        "Status updated",
    )
...
def get_favorites(
    ...
    other_members = (
        db.query(ConversationMember)
        .filter(
            ConversationMember.conversation_id.in_(member_convs),
...
def mark_notifications_read(
    ...
    for mem in db.query(ConversationMember).filter_by(user_id=current_user.id).all():
        last_msg = (
            db.query(Message)
            .filter_by(conversation_id=mem.conversation_id)
            .order_by(desc(Message.id))
            .first()
        )
        if last_msg:
    ...
    return success_response(
        {
            ...
            "online_status_visible": getattr(
                settings, "online_status_visible", "everyone"
            ),
...
def storage_dashboard(
    ...
    if not member_convs:
        return success_response(
            {"images": 0, "videos": 0, "files": 0, "audio": 0, "total": 0}
        )
