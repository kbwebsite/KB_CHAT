...
def _parse_future_time(raw):
    """Parse an ISO datetime, assuming UTC when naive. Returns None unless
    the value exists and lies in the future (naive-vs-aware comparison
    would otherwise raise TypeError and 500)."""
    if not raw:
        return None
    try:
...
def _member_ids(db, cid):
    return [
        m.user_id
        for m in db.query(ConversationMember).filter_by(conversation_id=cid).all()
    ]
...
async def create_scheduled(
    ...
    scheduled_at = _parse_future_time(payload.get("scheduled_at"))
    if not scheduled_at:
        raise HTTPException(
...
def list_scheduled(
    ...
    items = (
        ...
        .filter_by(conversation_id=conv_id, sender_id=current_user.id, sent=False)
...
def update_scheduled(
    ...
    sm = (
        db.query(ScheduledMessage)
        .filter_by(id=sm_id, sender_id=current_user.id)
        .first()
    )
    if not sm:
    ...
    if payload.get("scheduled_at"):
        new_time = _parse_future_time(payload.get("scheduled_at"))
        if not new_time:
            raise HTTPException(
                status_code=400, detail="Schedule time must be in the future"
            )
        sm.scheduled_at = new_time
    db.commit()
    return success_response(
        {
            "id": sm.id,
...
async def check_scheduled_messages():
    ...
    try:
        ...
        due = (
            db.query(ScheduledMessage)
            .filter(
                ScheduledMessage.sent == False, ScheduledMessage.scheduled_at <= now
            )
            .all()
        )
        for sm in due:
