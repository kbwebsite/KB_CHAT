...
export function CallsPanel({ onClose }: { onClose:()=>void }) {
  ...
  useEffect(()=>{
    callsApi.history()
      .then(r=>{ if(r.success) setCalls(r.data); else setError(r.message || 'Could not load calls') })
  ...
  return (
    <div className="h-full flex flex-col bg-card">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {loading ? <p className="text-sm text-muted-foreground p-4">Loading...</p> : error ? (
          ...
          <div key={c.id} className="panel-row flex items-center gap-3 p-3 rounded-xl bg-muted border">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate">{c.caller_id === meId ? c.callee_display : c.caller_display}</p>
